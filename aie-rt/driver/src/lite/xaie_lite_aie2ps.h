...
/**
*
* This API sets AXI-MM isolation boundry of an AI engine partition.
*
* @param	DevInst: Device Instance
*
...
*
******************************************************************************/
static inline void _XAie_LSetPartIsolationAxiMM(XAie_DevInst *DevInst)
{
	u64 RegAddr;
	u32 RegVal;
	for(u8 C = 0; C < DevInst->NumCols; C++) {
		RegVal = 0U;
		if(C == 0U) {
			RegVal = XAIE_PL_MOD_TILE_CNTR_AXI_MM_ISOLATE_FROM_WEST_MASK;
		}
		if(C == (DevInst->NumCols - 1)) {
			RegVal |= XAIE_PL_MOD_TILE_CNTR_AXI_MM_ISOLATE_FROM_EAST_MASK;
		}

		RegAddr  = _XAie_LGetTileAddr(0U, C) +
			XAIE_PL_MOD_TILE_CNTR_AXI_MM_ISOLATE_REGOFF;
