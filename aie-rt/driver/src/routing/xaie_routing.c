...
static XAie_LocType _XAie_dequeue(Queue* queue) {
    if (isEmpty(queue)) return (XAie_LocType){-1, -1}; // Invalid tile
...
static void freeQueue(Queue *queue) {
    if (queue != NULL) {
        if (queue->array != NULL) {
            free(queue->array); // Free the dynamically allocated array in the queue
        }
...
AieRC XAie_dumpConstraintsToPrint(XAie_RoutingInstance* RoutingInstance) {
    ...
    for (u8 row = 0; row < RoutingInstance->NumRows; row++) {
        for (u8 col = 0; col < RoutingInstance->NumCols; col++) {
            ...
            if (row < RoutingInstance->NumRows - 1 || col < RoutingInstance->NumCols - 1) {
                printf(",\n");
            }
        }
    }
...
AieRC XAie_dumpSpecificConstraintToPrint(XAie_RoutingInstance* RoutingInstance, u8 row, u8 col) {
    if (RoutingInstance == NULL || row >= RoutingInstance->NumRows || col >= RoutingInstance->NumCols) {
        XAIE_ERROR("XAie_dumpSpecificConstraintToPrint backend failed!. Invalid input or out of bounds row/col to dumpSpecificConstraintToPrint\n");
        return XAIE_ERR;
    }
...
******************************************************************************/
 static const char* _XAie_StrmSwPortTypeToString(StrmSwPortType type) {
    switch(type) {
        ...
        case SS_PORT_TYPE_MAX: return "SS_PORT_TYPE_MAX";
...
static void _XAie_drawRoute(XAie_RoutingInstance *routingInstance, XAie_LocType *path, int pathLength,
    ...
    char grid[routingInstance->NumRows][routingInstance->NumCols];
    ...
    for (int i = 0; i < pathLength; i++) {
        ...
        char mark = '*'; // Default mark for the path
        if (i == 0) {
            mark = 'S'; // Mark for source
        } else if (i == pathLength - 1) {
            mark = 'D'; // Mark for destination
            ...
            if (dRow == -1 && dCol == 0) mark = 'v'; // Down
            else if (dRow == 1 && dCol == 0) mark = '^'; // Up
            else if (dRow == 0 && dCol == -1) mark = '<'; // Left
            else if (dRow == 0 && dCol == 1) mark = '>'; // Right
        }

        if (gridCol < routingInstance->NumCols && gridRow >= 0 && gridRow < routingInstance->NumRows) {
    ...
    printf(" +"); // Top left corner of the box
    for (int col = 0; col < routingInstance->NumCols; col++) {
        printf("--"); // Top border of the box
    }
    ...
    printf(" +"); // Bottom left corner of the box
    for (int col = 0; col < routingInstance->NumCols; col++) {
        printf("--"); // Bottom border of the box
    }
...
AieRC XAie_RoutesReveal(XAie_RoutingInstance *routingInstance, XAie_LocType source, XAie_LocType destination) {
    XAie_CoreConstraint* constraint = routingInstance->CoreConstraintPerCore[source.Col][source.Row];
    if (constraint == NULL) {
        XAIE_ERROR("XAie_RoutesReveal backend failed!. Incorrect arguments!.\n");
        return XAIE_ERR;
    }

    XAie_RoutingPath *routingPath = _XAie_findRouteInRouteDB(constraint->routesDB, source, destination);

    if (routingPath == NULL) {
        XAIE_ERROR("XAie_RoutesReveal backend failed!. No Routing Path found between [%d %d] and [%d %d].\n", source.Col, source.Row, destination.Col, destination.Row);
        return XAIE_ERR;
    }

    char grid[routingInstance->NumRows][routingInstance->NumCols];
    memset(grid, '.', sizeof(grid)); // Initialize grid with '.'
    ...
    while (currentStep != NULL) {
        ...
        if (gridColSource < routingInstance->NumCols && gridRowSource >= 0 && gridRowSource < routingInstance->NumRows) {
...
static bool _XAie_isShimTile(XAie_RoutingInstance* RoutingInstance, XAie_LocType tile) {
    XAie_CoreConstraint* constraint = RoutingInstance->CoreConstraintPerCore[tile.Col][tile.Row];
...
*
******************************************************************************/
static AieRC _XAie_updatePortAvailabilityForShimDmaToAie(XAie_RoutingInstance *routingInstance, XAie_LocType tile, int port, bool SetOrUnset) {
    XAie_CoreConstraint* constraint = routingInstance->CoreConstraintPerCore[tile.Col][tile.Row];
...
*
******************************************************************************/
static AieRC _XAie_updatePortAvailabilityForAieToShimDma(XAie_RoutingInstance *routingInstance, XAie_LocType tile, int port, bool SetOrUnset) {
    XAie_CoreConstraint* constraint = routingInstance->CoreConstraintPerCore[tile.Col][tile.Row];
...
int findAvailableChannelIDforShimTile(XAie_RoutingInstance *routingInstance, XAie_LocType tile, u8 port, bool HostToAie)
{
    ...
    if (HostToAie)
    {
        for (int i = 0; i < constraint->Host2AIEPortCount; i++) {
            if (constraint->Host2AIEPorts[i].port == port) {
                return constraint->Host2AIEPorts[i].channel; // Found a matching stream
            }
        }
    }
    else {
        for (int i = 0; i < constraint->AIE2HostPortCount; i++) {
            if (constraint->AIE2HostPorts[i].port == port) {
                return constraint->AIE2HostPorts[i].channel; // Found a matching stream
            }
        }
    }
}
...
static int findFirstMatchingStreamForDestination(XAie_RoutingInstance* RoutingInstance,
    ...
    XAie_CoreConstraint* constraintSource = RoutingInstance->CoreConstraintPerCore[SourceTile.Col][SourceTile.Row];
    XAie_CoreConstraint* constraintDestination = RoutingInstance->CoreConstraintPerCore[DestinationTile.Col][DestinationTile.Row];
    if (!constraintSource || !constraintDestination) return -1; // If either constraint is null, return -1
    ...
    // Now find the first empty port that is available on both portAvailabilitySourceTile and portAvailabilityDestinationTile
    for (int i = 0; i < 8; i++) {
        if ((portAvailabilitySourceTile & (1 << i)) && (portAvailabilityDestinationTile & (1 << i))) {
            return i; // Found a matching stream
        }
    }

    XAIE_DBG("[ERROR] No available stream between tiles {%d %d} and {%d %d} in the direction %s\n", SourceTile.Col, SourceTile.Row,
                DestinationTile.Col, DestinationTile.Row,
                _XAie_StrmSwPortTypeToString(SourceDirection));
    return -1; // No available or matching stream found
}
...
int _XAie_findFirstMatchingStream(XAie_RoutingInstance* RoutingInstance, XAie_LocType tile,
 StrmSwPortType direction, bool isEndTile) {
    ...
    if (_XAie_isShimTile(RoutingInstance, tile)) {
        // For Shim to AIE based connections, communication can only be done on very specific ports
        if (isEndTile)
        {
            for (int i = 0; i < constraint->AIE2HostPortCount; i++) {
                if (constraint->AIE2HostPorts[i].availability) {
                    return constraint->AIE2HostPorts[i].port; // Found a available port
                }
            }
        }
        else
        {
            for (int i = 0; i < constraint->Host2AIEPortCount; i++) {
                if (constraint->Host2AIEPorts[i].availability) {
                    return constraint->Host2AIEPorts[i].port; // Found a available port
                }
            }
        }
    }
    ...
    for (int i = 0; i < 8; i++) {
        if (portAvailability & (1 << i)) {
            return i; // Return the port number
        }
    }

    XAIE_DBG("[ERROR] No available stream found for {%d %d} in the direction %s\n", tile.Col, tile.Row,
            _XAie_StrmSwPortTypeToString(direction));
    return -1; // No available or matching stream found
}
...
static void _XAie_resetBDAvailability(XAie_RoutingInstance *routingInstance, XAie_LocType tile, u8 bdID) {
    ...
    const int MAX_BUFFER_IDS = 16; // Adjust this value based on your system's specification
...
static int _XAie_findAvailableBufferID(XAie_RoutingInstance *routingInstance, XAie_LocType tile) {
    ...
    const int MAX_BUFFER_IDS = 48; // MemTile has a max of 48 available BDs
...
static bool _XAie_isAdjTileValidForCurrTile(XAie_RoutingInstance* RoutingInstance, XAie_RouteConstraints* RouteConstraints,
    ...
    if (!(adjTile.Col >= 0 && adjTile.Col < RoutingInstance->NumCols &&
          adjTile.Row >= 0 && adjTile.Row < RoutingInstance->NumRows))
        return false;

    // Check port availability in both current and adjacent tile
    bool isPortAvailable = false;
    if (coreConstraints[currentTile.Col][currentTile.Row] && coreConstraints[adjTile.Col][adjTile.Row]) {
        XAie_CoreConstraint *currentConstraint = coreConstraints[currentTile.Col][currentTile.Row];
        XAie_CoreConstraint *adjConstraint = coreConstraints[adjTile.Col][adjTile.Row];

        switch (direction) {
            case NORTH: // North
                isPortAvailable = currentConstraint->SlaveNorth && adjConstraint->MasterSouth;
                break;
            case SOUTH: // South
                isPortAvailable = currentConstraint->SlaveSouth && adjConstraint->MasterNorth;
                break;
            case EAST: // East
                isPortAvailable = currentConstraint->SlaveEast && adjConstraint->MasterWest;
                break;
            case WEST: // West
    ...
    return !_XAie_isTileBlackListed(adjTile, RouteConstraints) // check if adjacent tile is blacklisted
...
AieRC XAie_RoutingSwitchReset(XAie_RoutingInstance *routingInstance, XAie_LocType* listOfTiles, uint32_t NoOfTiles) {
    ...
    for (uint32_t i = 0; i < NoOfTiles; i++) {
        ...
        XAie_CoreConstraint* tileConstraint = routingInstance->CoreConstraintPerCore[currentTile.Col][currentTile.Row];

        // Check if the tile is auto-configured; if so, skip resetting
        if (tileConstraint->isAutoConfigured) {
            XAIE_DBG("XAie_RoutingSwitchReset Warning: Skipping auto-configured tile at {%d, %d}.\n", currentTile.Col, currentTile.Row);
            continue; // Skip this tile
        }

        StrmSwPortType directions[] = {DMA, SOUTH, WEST, NORTH, EAST};
        int dirCount = sizeof(directions) / sizeof(directions[0]); // Number of elements in the directions array
        ...
        for (int slaveIdx = 0; slaveIdx < dirCount; ++slaveIdx) {
            ...
            switch (slaveDir) {
                case SOUTH: slavePortBitfield = tileConstraint->SlaveSouth; break;
                case NORTH: slavePortBitfield = tileConstraint->SlaveNorth; break;
            ...
            for (int masterIdx = 0; masterIdx < dirCount; ++masterIdx) {
                ...
                switch (masterDir) {
                    case SOUTH: masterPortBitfield = tileConstraint->MasterSouth; break;
                    case NORTH: masterPortBitfield = tileConstraint->MasterNorth; break;
                ...
                for (u8 bit = 0; bit < 8; bit++) { // Assuming 8 bits for simplicity; adjust as needed
                    if (slavePortBitfield & (1 << bit)) { // Check if slave port bit is set
                        for (u8 bitM = 0; bitM < 8; bitM++) { // Repeat for master port
                            if (masterPortBitfield & (1 << bitM)) { // Check if master port bit is set
                                //since we are brute forcing our way, we may stumble upon invalid configurations
                                //The below API is a (silenced) version of "XAie_StrmConnCctDisable" that gracefully skips those configurations without
                                //erroring out.
                                rc = XAie_SStrmConnCctDisable(routingInstance->DeviceInstance, currentTile, slaveDir, bit, masterDir, bitM);
                                if (rc != XAIE_OK) {
                                    XAIE_ERROR("XAie_RoutingSwitchReset Error: Failed to disable stream connection at Col: %d, Row: %d, Slave Dir: %s, Slave Port: %d, Master Dir: %s, Master Port: %d.\n", currentTile.Col, currentTile.Row, slaveDir, bit, masterDir, bitM);
                                    return XAIE_ERR;
                                }
                            }
                        }
...
void XAie_DumpRoutingSwitchInfo(XAie_RoutingInstance *routingInstance, XAie_LocType* listOfTiles, uint32_t NoOfTiles) {
    ...
    for (int i = 0; i < NoOfTiles; i++) {
        ...
        printf("\tMM2S_State: "); PrintBits(CurrentTileconstraint->MM2S_State); printf("\n");
        printf("\tS2MM_State: "); PrintBits(CurrentTileconstraint->S2MM_State); printf("\n");
        if (CurrentTileconstraint->tile_type == XAIE_AIE_SHIM)
        {
            printf("\tShimMM2S_State: "); PrintBits(CurrentTileconstraint->ShimMM2S_State); printf("\n");
            printf("\tShimS2MM_State: "); PrintBits(CurrentTileconstraint->ShimS2MM_State); printf("\n");

        }
        printf("\tBDState: "); PrintBDBits(CurrentTileconstraint->BDState); printf("\n");

        // Slave and Master ports
        printf("\tSlaveEast: "); PrintBits(CurrentTileconstraint->SlaveEast); printf("\n");
        printf("\tSlaveWest: "); PrintBits(CurrentTileconstraint->SlaveWest); printf("\n");
        printf("\tSlaveSouth: "); PrintBits(CurrentTileconstraint->SlaveSouth); printf("\n");
        printf("\tSlaveNorth: "); PrintBits(CurrentTileconstraint->SlaveNorth); printf("\n");
        printf("\tMasterEast: "); PrintBits(CurrentTileconstraint->MasterEast); printf("\n");
        printf("\tMasterWest: "); PrintBits(CurrentTileconstraint->MasterWest); printf("\n");
        printf("\tMasterSouth: "); PrintBits(CurrentTileconstraint->MasterSouth); printf("\n");
        printf("\tMasterNorth: "); PrintBits(CurrentTileconstraint->MasterNorth); printf("\n");
        ...
        while (currentRoute != NULL) {
            ...
            printf("\tSource[%d, %d] -> Destination[%d, %d], MM2S_portNo: %d, S2MM_portNo: %d\n",
            ...
            while (step != NULL) {
                ...
                printf("\t| Source Stream: %d, Direction: %s\n", step->sourceStream, _XAie_StrmSwPortTypeToString(step->source_direction));
                printf("\t| Dest Stream: %d, Direction: %s\n", step->destStream, _XAie_StrmSwPortTypeToString(step->dest_direction));
                printf("\t------------------------------------------------\n");
                step = step->next;
                stepCount++;
            }
...
AieRC XAie_DeRoute(XAie_RoutingInstance *routingInstance, XAie_LocType source,
    ...
    XAie_CoreConstraint* Sourceconstraint = routingInstance->CoreConstraintPerCore[source.Col][source.Row];
    XAie_CoreConstraint* Destconstraint = routingInstance->CoreConstraintPerCore[destination.Col][destination.Row];
    ...
    if (Sourceconstraint == NULL || Sourceconstraint->routesDB == NULL) {
        XAIE_ERROR("XAie_DeRoute failed!. Routing Path does not exist between Source {%d, %d} and Destination {%d, %d}\n",
         source.Col, source.Row, destination.Col, destination.Row);
        return XAIE_ERR; // Routing path does not exist
    }

    if (ShouldModifyCoreConfig && !_XAie_isShimTile(routingInstance, source) && !_XAie_isMemTile(routingInstance, source))
        Sourceconstraint->isCoreExecuting = false;

    if (ShouldModifyCoreConfig && !_XAie_isShimTile(routingInstance, destination) && !_XAie_isMemTile(routingInstance, destination))
        Destconstraint->isCoreExecuting = false;

    XAie_RoutingPath *routingPath = _XAie_findRouteInRouteDB(Sourceconstraint->routesDB, source, destination);
    ...
    while (currentStep != NULL) {
        ...
        if (RC != XAIE_OK) {
            XAIE_ERROR("\n [COMMAND] XAie_StrmConnCctDisable({%d,%d}, %s, %d, %s, %d) FAILED!!\n",
            currentStep->sourceTile.Col, currentStep->sourceTile.Row,
            _XAie_StrmSwPortTypeToString(currentStep->source_direction),
            currentStep->sourceStream,
            _XAie_StrmSwPortTypeToString(currentStep->dest_direction),
        ...
        _XAie_updatePortAvailabilityForStrmConnInverse(routingInstance, currentStep->sourceTile,
                                                 currentStep->source_direction, currentStep->sourceStream,
                                                 currentStep->dest_direction, currentStep->destStream);
        ...
        if (_XAie_isShimTile(routingInstance, routingPath->source))
        {
            ...
            if (currentStep == routingPath->nextStep)
            {
                ...
                // int RC = XAie_DisableShimDmaToAieStrmPort(DevInst, currentStep->sourceTile,
                //                     currentStep->sourceStream);

                // XAIE_DBG("\nXAie_DisableShimDmaToAieStrmPort({%d,%d}, %d)\n",
                // currentStep->sourceTile.Col, currentStep->sourceTile.Row
                // currentStep->sourceStream);
                _XAie_updatePortAvailabilityForShimDmaToAie(routingInstance, currentStep->sourceTile, currentStep->sourceStream, true);
            }
            else
            {
                //currently there is no API to disable ShimDMA to AIEStream Port
...
static AieRC _XAie_performRoutingOnPath(XAie_RoutingInstance *routingInstance, XAie_LocType source, XAie_LocType destination, XAie_LocType *path, int pathLength) {
    XAie_DevInst *DevInst = routingInstance->DeviceInstance;
    int RC = XAIE_OK; // Assuming XAIE_OK is a constant representing successful operation
    StrmSwPortType lastDir = SOUTH; // Used to store the last destination stream
    int lastStream = -1;
    XAie_CoreConstraint* Sourceconstraint = routingInstance->CoreConstraintPerCore[source.Col][source.Row];
    ...
    CurrRoutingPath->MM2S_portNo = -1; CurrRoutingPath->S2MM_portNo = -1;
    ...
    for (int i = 0; i < pathLength; i++) {
        if (i == pathLength - 1) {
            ...
            StrmSwPortType dirLast = (_XAie_isShimTile(routingInstance, LastTile)) ? SOUTH : DMA;
            int destStream = _XAie_findFirstMatchingStream(routingInstance, LastTile, dirLast, true/*is End Tile*/);

            if (destStream == -1)
            {
                XAIE_ERROR("Routing Failed. No available port on AIE-Tile {%d, %d} in the direction of %s. Please check constraints and consider re-route!\n", LastTile.Col, LastTile.Row, _XAie_StrmSwPortTypeToString(dirLast));
                return XAIE_ERR;
            }

            // set current tile as autoconfigured
            XAie_CoreConstraint* CurrentTileconstraint = routingInstance->CoreConstraintPerCore[LastTile.Col][LastTile.Row];
            ...
            RC |= XAie_StrmConnCctEnable(DevInst, LastTile, lastDir, lastStream, dirLast, destStream);
            if (RC != XAIE_OK) {
                XAIE_ERROR("Routing Failed!. XAie_StrmConnCctEnable Failed!\n");
                return XAIE_ERR;
            }

            _XAie_updatePortAvailabilityForStrmConn(routingInstance, LastTile, lastDir, lastStream, dirLast, destStream);
            ...
            if (_XAie_isShimTile(routingInstance, LastTile)) {
                // Print the command and parameters before executing
                XAIE_DBG("\n [COMMAND] XAie_EnableAieToShimDmaStrmPort({%d,%d}, %d)\n",
                    LastTile.Col, LastTile.Row, destStream);

                // Call the function
                RC |= XAie_EnableAieToShimDmaStrmPort(DevInst, LastTile, destStream);
                if (RC != XAIE_OK) {
                    XAIE_ERROR("Routing Failed!. XAie_EnableAieToShimDmaStrmPort Failed!\n");
                    return XAIE_ERR;
                }
                _XAie_updatePortAvailabilityForAieToShimDma(routingInstance, LastTile, destStream, false);
            }

            if (Sourceconstraint->routesDB == NULL)
            {
                XAIE_DBG("Adding first route to routes DB\n");
                Sourceconstraint->routesDB = aligned_alloc(16,sizeof(XAie_ProgrammedRoutes));
                Sourceconstraint->routesDB->routePath = CurrRoutingPath;
            }
            else
                _XAie_addRouteInRoutesDB(&(Sourceconstraint->routesDB), CurrRoutingPath);
        }
        else{
            ...
            {
                if (i == 0)
                    sourceStream = _XAie_findFirstMatchingStream(routingInstance, SourceTile, actualdirSource, false);
                else
            ...
            int destStream = findFirstMatchingStreamForDestination(routingInstance, SourceTile, DestinationTile,
                            dirSource);
            ...
            if (sourceStream == -1 || destStream == -1)
            {
                ...
                XAIE_ERROR("\n XAie_StrmConnCctEnable cannot be programmed between [%d %d] and"
                         "[%d %d] due to unavailable stream ports. Please check constraints and consider re-route!\n",
                SourceTile.Col, SourceTile.Row,
                DestinationTile.Col, DestinationTile.Row);
                return XAIE_ERR;
            }
            ...
            // set current tile as autoconfigured
            XAie_CoreConstraint* CurrentTileconstraint = routingInstance->CoreConstraintPerCore[SourceTile.Col][SourceTile.Row];
            CurrentTileconstraint->isAutoConfigured = true;
            ...
            RC |= XAie_StrmConnCctEnable(DevInst, SourceTile, portSource, sourceStream, portDest, destStream);
            if (RC != XAIE_OK) {
                XAIE_ERROR("Routing Failed!. XAie_EnableAieToShimDmaStrmPort Failed!\n");
                return XAIE_ERR;
            }
            ...
            _XAie_updatePortAvailabilityForStrmConn(routingInstance, SourceTile, portSource, sourceStream, portDest, destStream);
            ...
            if (_XAie_isShimTile(routingInstance, SourceTile) && (i == 0)){
                // Print the command and parameters before executing
                XAIE_DBG("\n [COMMAND] XAie_EnableShimDmaToAieStrmPort({%d,%d}, %d)\n",
                    SourceTile.Col, SourceTile.Row, sourceStream);

                // Call the function
                RC |= XAie_EnableShimDmaToAieStrmPort(DevInst, SourceTile, sourceStream);
                if (RC != XAIE_OK) {
                    XAIE_ERROR("Routing Failed!. XAie_EnableShimDmaToAieStrmPort Failed!\n");
                    return XAIE_ERR;
                }
                _XAie_updatePortAvailabilityForShimDmaToAie(routingInstance, SourceTile, sourceStream, false);
            }
        }
    }
...
static AieRC _XAie_programBufferDescriptors(XAie_RoutingInstance *routingInstance, XAie_LocType source, void* SourceObject, u32 data_size, void* DestinationObject,
    ...
    XAIE_DBG("\n [COMMAND] XAie_DmaDescInit(&SourceBufferDescriptor, {%d,%d})\n",source.Col, source.Row);
    if (_XAie_isShimTile(routingInstance, source)) {
        if ((DevInst->Backend->Type == XAIE_IO_BACKEND_BAREMETAL) || (DevInst->Backend->Type == XAIE_IO_BACKEND_SOCKET))
            RC |= XAie_DmaSetAddrLen(&SourceBufferDescriptor, (u64)SourceObject, data_size);
        else
            RC |= XAie_DmaSetAddrOffsetLen(&SourceBufferDescriptor, (XAie_MemInst*)SourceObject, 0x0, data_size);
    } else {
        RC |= XAie_DmaSetAddrLen(&SourceBufferDescriptor, (u64)(void*)SourceObject, data_size);
    }
    ...
    XAIE_DBG("\n [COMMAND] XAie_DmaDescInit(&DestBufferDescriptor, {%d,%d})\n",destination.Col, destination.Row);
    if (_XAie_isShimTile(routingInstance, destination)) {
        if ((DevInst->Backend->Type == XAIE_IO_BACKEND_BAREMETAL) || (DevInst->Backend->Type == XAIE_IO_BACKEND_SOCKET))
            RC |= XAie_DmaSetAddrLen(&DestBufferDescriptor, (u64)DestinationObject, data_size);
        else
            RC |= XAie_DmaSetAddrOffsetLen(&DestBufferDescriptor, (XAie_MemInst*)DestinationObject, 0x0, data_size);
    } else {
        RC |= XAie_DmaSetAddrLen(&DestBufferDescriptor, (u64)(void*)DestinationObject, data_size);
    }
...
*
******************************************************************************/
AieRC XAie_MoveData(XAie_RoutingInstance *routingInstance, XAie_LocType source,
    void* SourceObject, u32 data_size, void* DestinationObject, XAie_LocType destination) {
    ...
    if (_XAie_programBufferDescriptors(routingInstance, source, SourceObject,
        data_size, DestinationObject, destination, &bufferDescriptors) != XAIE_OK) {
    ...
    if (routingPath == NULL)
    {
        XAIE_ERROR("no programmed path found between Source: {%d %d} and Destination {%d %d}\n",
                     source.Col, source.Row, destination.Col, destination.Row);
        return XAIE_ERR;
    }
    ...
    RC |= XAie_DmaChannelPushBdToQueue(DevInst, source, sourceChannelID, DMA_MM2S, bufferDescriptors.sourceBD);
    ...
    MM2S.MM2S_ports = realloc(MM2S.MM2S_ports,
                                    (MM2S_Count + 1) * sizeof(u8));
    MM2S.MM2S_ports[MM2S_Count] = sourceChannelID;

    // Repeat the same process for destination
    XAIE_DBG("\n [COMMAND] XAie_DmaChannelPushBdToQueue({%d, %d}, %d, DMA_S2MM, %d)\n", destination.Col, destination.Row, destChannelID, bufferDescriptors.destinationBD);
    XAIE_DBG("\n [COMMAND] XAie_DmaChannelEnable({%d,%d}, %d, DMA_S2MM)\n", destination.Col, destination.Row, destChannelID);
    RC |= XAie_DmaChannelPushBdToQueue(DevInst, destination, destChannelID, DMA_S2MM, bufferDescriptors.destinationBD);
    ...
    S2MM.S2MM_ports = realloc(S2MM.S2MM_ports,
                                    (S2MM_Count + 1) * sizeof(u8));
...
static bool _XAie_findShortestPath(XAie_RoutingInstance *routingInstance, XAie_RouteConstraints* RouteConstraints,
XAie_LocType source, XAie_LocType destination, XAie_LocType *path, int *pathLength) {
    ...
    bool **visited = (bool**)aligned_alloc(16,MAX_COLS * sizeof(bool*));
    ...
    Queue* queue = createQueue(MAX_ROWS * MAX_COLS);
    if (queue != NULL) {
    ...
    XAie_LocType pred[MAX_COLS][MAX_ROWS]; // To store the predecessors of each tile
    memset(pred, 0, sizeof(pred)); // Initialize predecessors
    bool found = false;
    ...
    while (!isEmpty(queue) && !found) {
        ...
        for (int i = 0; i < 4; i++) {
            XAie_LocType adj = XAie_TileLoc(current.Col + dCol[i], current.Row + dRow[i]);

            if (_XAie_isAdjTileValidForCurrTile(routingInstance, RouteConstraints, current, adj, StrmSwPort[i], visited)) {
                ...
                if (adj.Col == destination.Col && adj.Row == destination.Row) {
                    ...
                    while (!(temp.Col == source.Col && temp.Row == source.Row)) {
                        if (RouteConstraints && RouteConstraints->NoOfWhiteListedCores > 0
                                    && !_XAie_isTileWhitelisted(temp, RouteConstraints->WhiteListedCores, RouteConstraints->NoOfWhiteListedCores)) {
...
AieRC XAie_Run(XAie_RoutingInstance* routingInstance, uint32_t count) {
    ...
    for (int i = 0; i < count; i++)
    {
        for (u8 col = 0; col < routingInstance->NumCols; col++) {
            for (u8 row = 0; row < routingInstance->NumRows; row++) {
                XAie_CoreConstraint* coreConstraint = routingInstance->CoreConstraintPerCore[col][row];
                if (coreConstraint != NULL && coreConstraint->isCoreExecuting) {
                    XAie_CoreEnable(routingInstance->DeviceInstance,  XAie_TileLoc(col, row));
...
{
    XAie_CoreConstraint* coreConstraint = routingInstance->CoreConstraintPerCore[tile.Col][tile.Row];
    XAie_s2mmChannelsInUse S2MM_val = coreConstraint->s2mmChannelsInUse;
    XAie_mm2sChannelsInUse MM2S_val = coreConstraint->mm2sChannelsInUse;
...
{
    //iterate through all the user specified constraints and configure appropriate constraints on the appropriate SHIM Tiles
    for (int i = 0; i < UserResCon.ConstraintCount; i++)
    {
        ...
        XAie_CoreConstraint* coreConstraint = routingInstance->CoreConstraintPerCore[CurrTile.Col][CurrTile.Row];
...
AieRC XAie_Route(XAie_RoutingInstance *routingInstance,  XAie_RouteConstraints* RouteConstraints,
    ...
    {
        XAIE_ERROR("XAie_Route backend failed!. Route has already been programmed between source and destination\n");
        return XAIE_ERR;
    }

    XAie_LocType *path = aligned_alloc(16,routingInstance->NumRows * routingInstance->NumCols * sizeof(XAie_LocType));
    if (!path) {
    ...
    if (!_XAie_findShortestPath(routingInstance, RouteConstraints, source, destination, path, &pathLength)) {
        free(path);
		XAIE_ERROR("XAie_Route backend Failed!. Unable to find path between [%d %d] and [%d %d].\n", source.Col, source.Row, destination.Col, destination.Row);
        return XAIE_ERR;
    }

    // Perform routing along the found path
    if(_XAie_performRoutingOnPath(routingInstance, source, destination, path, pathLength) != XAIE_OK)
    {
        free(path);
		XAIE_ERROR("XAie_Route backend Failed!. Unable to program path between [%d %d] and [%d %d].\n", source.Col, source.Row, destination.Col, destination.Row);
        return XAIE_ERR;
    }

    if ((!_XAie_isShimTile(routingInstance, source)) && (!_XAie_isMemTile(routingInstance, source)))
        XAie_SetCoreExecute(routingInstance, source, true);

    if ((!_XAie_isShimTile(routingInstance, destination)) && (!_XAie_isMemTile(routingInstance, destination)))
...
*
******************************************************************************/
static bool _XAie_isTileBlackListed(XAie_LocType CurrTile, XAie_RouteConstraints* initialConstraints)
{
...
*
******************************************************************************/
XAie_RoutingInstance* XAie_InitRoutingHandler(XAie_DevInst *DevInst) //DeviceInstance[routing information]
{
        ...
        RoutingInstance->CoreConstraintPerCore = (XAie_CoreConstraint***)aligned_alloc(16,NumCols * sizeof(XAie_CoreConstraint**));

        if (RoutingInstance->CoreConstraintPerCore == NULL) {
            XAIE_ERROR("XAie_InitRoutingHandler backend failed!. Failed to allocate memory to CoreConstraintPerCore\n");
		    return NULL;
        }

        for (u8 col = 0; col < NumCols; col++) {
            // Allocate memory for each column in a col
            RoutingInstance->CoreConstraintPerCore[col] = (XAie_CoreConstraint**)aligned_alloc(16,NumRows * sizeof(XAie_CoreConstraint*));
            if (RoutingInstance->CoreConstraintPerCore[col] == NULL) {
                XAIE_ERROR("XAie_InitRoutingHandler backend failed!. Failed to allocate memory to CoreConstraintPerCore[%d] \n", col);
		        return NULL;
            }

            for (u8 row = 0; row < NumRows; row++) {
                // Allocate memory for each core constraint
                RoutingInstance->CoreConstraintPerCore[col][row] = (XAie_CoreConstraint*)aligned_alloc(16,sizeof(XAie_CoreConstraint));
            }
        }

        XAie_ChannelPortMapping* defaultHost2AIEPortChannelMapping = aligned_alloc(16,2*sizeof(XAie_ChannelPortMapping));
        XAie_ChannelPortMapping* defaultAIE2HostPortChannelMapping = aligned_alloc(16,2*sizeof(XAie_ChannelPortMapping));

        defaultHost2AIEPortChannelMapping[0].port = 3; defaultHost2AIEPortChannelMapping[0].channel = 0; defaultHost2AIEPortChannelMapping[0].availability = true;
        defaultHost2AIEPortChannelMapping[1].port = 7; defaultHost2AIEPortChannelMapping[1].channel = 1; defaultHost2AIEPortChannelMapping[1].availability = true;

        if(DevInst->DevProp.DevGen == XAIE_DEV_GEN_AIE2PS)
        {
            defaultAIE2HostPortChannelMapping[0].port = 1; defaultAIE2HostPortChannelMapping[0].channel = 0; defaultAIE2HostPortChannelMapping[0].availability = true;
            defaultAIE2HostPortChannelMapping[1].port = 3; defaultAIE2HostPortChannelMapping[1].channel = 1; defaultAIE2HostPortChannelMapping[1].availability = true;
        }
        else
        {
            defaultAIE2HostPortChannelMapping[0].port = 2; defaultAIE2HostPortChannelMapping[0].channel = 0; defaultAIE2HostPortChannelMapping[0].availability = true;
            defaultAIE2HostPortChannelMapping[1].port = 3; defaultAIE2HostPortChannelMapping[1].channel = 1; defaultAIE2HostPortChannelMapping[1].availability = true;
        }
        ...
        for (u8 row = 0; row < NumRows; row++)
            for (u8 col = 0; col < NumCols; col++)
            {
                    ...
                    XAie_CoreConstraint *CurrCoreConstraint = RoutingInstance->CoreConstraintPerCore[col][row];

                    //there are 4 channels. 2-S2MM and 2-MM2S, initialize all of these channels to available [1]. Rest unavailable [0]
                    if (isMemTileRow && !isShimTileRow) //sometimes if no memtile, then memtile row by default would be 0 --> This is not memtile then, its shim tile
                    {
                        CurrCoreConstraint->isAutoConfigured = false;
                        CurrCoreConstraint->tile_type = 1;
                        CurrCoreConstraint->S2MM_State = 0x3F; //set 6 bits and rest 0
                        CurrCoreConstraint->MM2S_State = 0x3F; //set 6 bits and rest 0
                        CurrCoreConstraint->ShimS2MM_State = 0x0;
                        CurrCoreConstraint->ShimMM2S_State = 0x0;
                        CurrCoreConstraint->BDState = 0xFFFFFFFFFFFF; //48 BDs are available
                        //only north and south streams accessible
                        CurrCoreConstraint->DirSupported = (XAie_StreamDirSupported){.canMoveNorth = 1, .canMoveSouth = 1, .canMoveEast = 0, .canMoveWest = 0};
                        CurrCoreConstraint->SlaveEast = 0x0;//no east streaming
                        CurrCoreConstraint->SlaveWest = 0x0; //no west streaming
                        CurrCoreConstraint->SlaveSouth = 0x3F; // 6 slave ports on south
                        CurrCoreConstraint->SlaveNorth = 0xF; // 4 slave ports on north

                        CurrCoreConstraint->MasterEast = 0x0;//no east streaming
                        CurrCoreConstraint->MasterWest = 0x0; //no west streaming
                        CurrCoreConstraint->MasterSouth = 0xF; // 4 slave ports on south
                        CurrCoreConstraint->MasterNorth = 0x3F; // 6 slave ports on north
                    ...
                    else if (isAieTile)
                    {
                        ...
                        CurrCoreConstraint->S2MM_State = 0x3; //set 2 bits and rest 0
                        CurrCoreConstraint->MM2S_State = 0x3; //set 2 bits and rest 0
                        CurrCoreConstraint->ShimS2MM_State = 0x0;
                        CurrCoreConstraint->ShimMM2S_State = 0x0;
                        CurrCoreConstraint->BDState = 0xFFFF; //16 BDs are available
                        //north, south, west and east streams accessible
                        CurrCoreConstraint->DirSupported = (XAie_StreamDirSupported){.canMoveNorth = 1, .canMoveSouth = 1, .canMoveEast = 1, .canMoveWest = 1};

                        CurrCoreConstraint->SlaveEast = 0xF;// 4 ports on the EAST
                        CurrCoreConstraint->SlaveWest = 0xF; // 4 ports on the WEST
                        CurrCoreConstraint->SlaveSouth = 0x3F; // 6 slave ports on south
                        CurrCoreConstraint->SlaveNorth = 0xF; // 4 slave ports on north

                        CurrCoreConstraint->MasterEast = 0xF;// 4 ports on the EAST
                        CurrCoreConstraint->MasterWest = 0xF; // 4 ports on the WEST
                        CurrCoreConstraint->MasterSouth = 0xF; // 4 slave ports on south
                        CurrCoreConstraint->MasterNorth = 0x3F; // 6 slave ports on north
                    ...
                    else if (isShimTileRow)
                    {
                        ...
                        CurrCoreConstraint->S2MM_State = 0x3; //2 S2MM channels?
                        CurrCoreConstraint->MM2S_State = 0x3; //2 MM2S channels?
                        ...
                        CurrCoreConstraint->SlaveEast = 0xF;// 4 ports on the EAST
                        CurrCoreConstraint->SlaveWest = 0xF; // 4 ports on the WEST
                        CurrCoreConstraint->SlaveSouth = 0x0; // SHIM tiles dont stream down
                        CurrCoreConstraint->SlaveNorth = 0xF; // 4 slave ports on north

                        CurrCoreConstraint->MasterEast = 0xF;// 4 ports on the EAST
                        CurrCoreConstraint->MasterWest = 0xF; // 4 ports on the WEST
                        CurrCoreConstraint->MasterSouth = 0x0; // SHIM tiles dont stream down
                        CurrCoreConstraint->MasterNorth = 0x3F; // 6 slave ports on north

                        CurrCoreConstraint->Host2AIEPorts = defaultHost2AIEPortChannelMapping; //DEFAULT: ports 3 and 7 are available by default to move data from SHIM DMA to AIE Tile
                        CurrCoreConstraint->Host2AIEPortCount = 2; //DEFAULT: 2 channels available at 0 and 1
                        CurrCoreConstraint->AIE2HostPorts = defaultAIE2HostPortChannelMapping;
            ...
            for (u8 row = 0; row < NumRows; row++) {
                ...
                if (coreConstraint != NULL) {
                    ...
                    if (coreConstraint->routesDB != NULL) {
                        _XAie_freeXAieProgrammedRoutes(coreConstraint->routesDB);
...
static XAie_RoutingPath* _XAie_findRouteInRouteDB(XAie_ProgrammedRoutes* routesDB, XAie_LocType source, XAie_LocType destination) {
    ...
    while ((CurrRoute != NULL) &&  (CurrRoute->routePath != NULL)){
        if ((CurrRoute->routePath->source.Col == source.Col && CurrRoute->routePath->source.Row == source.Row)
             && (CurrRoute->routePath->destination.Col == destination.Col && CurrRoute->routePath->destination.Row == destination.Row)){
            return CurrRoute->routePath; // Route founds
        }
...
static void _XAie_FreeRouteFromRoutesDB(XAie_ProgrammedRoutes** head, XAie_LocType source, XAie_LocType destination) {
    ...
    while (temp != NULL && !((temp->routePath->source.Col == source.Col && temp->routePath->source.Row == source.Row) &&
         (temp->routePath->destination.Col == destination.Col && temp->routePath->destination.Row == destination.Row))) {
        prev = temp;
        temp = temp->nextRoute;
    }
...
static void _XAie_freeXAieProgrammedRoutes(XAie_ProgrammedRoutes* headRoute) {
    while (headRoute != NULL) {
        ...
        XAie_RoutingPath* path = headRoute->routePath; // Free the routing path associated with this route
        if (path != NULL) {
            XAie_RoutingStep* step = path->nextStep; // Free all steps in the path
            while (step != NULL) {
                XAie_RoutingStep* nextStep = step->next;
                free(step);
                step = nextStep;
            }
            free(path); // Then free the path itself
        }
        free(headRoute); // Then free the route node itself
        headRoute = nextRoute;
    }
}
