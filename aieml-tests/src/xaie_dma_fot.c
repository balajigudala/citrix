...
* @{
*
* This file contains the test application to show case FoT feature of AIE DMA
*
* This application write random integers to memory buffer on Tile_1. Tile_2
* is configured to receive NUM_ELEMS_MAX integers on S2MM channel 0 and the FoT
* feature is enabled on the same channel. NUM_ELEMS_MAX / 2 is sent from Tile_1
* to Tile_2. Tile_2 DMA S2MM channel 0 terminates after receiving
* NUM_ELEMS_MAX / 2 integers. Data is checked for correctness.
*
******************************************************************************/
/***************************** Include Files *********************************/
#include <stdio.h>
...
int test_aie_dma_fot(XAie_DevInst *DevInst)
{
	AieRC RC = XAIE_OK;
	uint32_t Count;
	...
	/*
	 * FIXME: Replace read32 with actual API to get the count once it is
	 * available.
	 */
	XAie_Read32(DevInst,
			_XAie_GetTileAddr(DevInst, Tile_2.Row, Tile_2.Col) +
			0x0001DF18, &Count);
	if(Count != NUM_ELEMS_MAX / 2) {
		printf("Data transfer count is not %d 32 bit words\n",
				NUM_ELEMS_MAX / 2);
		printf("AIE DMA FoT failed.\n");
		return -1;
	}
	...
	printf("AIE DMA FoT success.\n");

	return 0;
}

int test_mem_tile_dma_fot(XAie_DevInst *DevInst)
{
	AieRC RC = XAIE_OK;
	uint32_t Count;
	uint32_t data[NUM_ELEMS_MAX];
	 ...
	 */
	XAie_Read32(DevInst,
			_XAie_GetTileAddr(DevInst, MemTile.Row, MemTile.Col) +
			0x000A06B0, &Count);
	if(Count != NUM_ELEMS_MAX / 2) {
		printf("Data transfer count is not %d 32 bit words\n",
				NUM_ELEMS_MAX / 2);
		printf("AIE Mem Tile DMA FoT failed.\n");
		return -1;
	}
	...
	printf("AIE Mem Tile DMA FoT success.\n");
